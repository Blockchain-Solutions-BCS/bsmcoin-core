...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total bsmcoin balance of the wallet\n"
    ...
    UniValue operator()(const CScriptID &scriptID) const {
        ...
        if (pwalletMain && pwalletMain->GetCScript(scriptID, subscript)) {
            ...
            for(const CTxDestination& addr: addresses)
                a.push_back(CBsmCoinAddress(addr).ToString());
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"bsmcoinaddress\"\n"
            "\nReturn information about the given bsmcoin address.\n"
            "\nArguments:\n"
            "1. \"bsmcoinaddress\"     (string, required) The bsmcoin address to validate. This may also be an OpenAlias address.\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"bsmcoinaddress\", (string) The bsmcoin address validated\n"
            "  \"scriptPubKey\" : \"hex\",       (string) The hex encoded scriptPubKey generated by the address\n"
            "  \"stakingaddress\" : \"bsmcoinaddress\", (string) The bsmcoin staking address part of a cold staking address\n"
            "  \"spendingaddress\" : \"bsmcoinaddress\", (string) The bsmcoin spending address part of a cold staking address\n"
            "  \"votingaddress\" : \"bsmcoinaddress\", (string) The bsmcoin voting address part of a cold staking v2 address\n"
    ...
    if (isValid)
    {
        ...
        if (address.IsColdStakingAddress(Params())) {
            CBsmCoinAddress stakingAddress;
            address.GetStakingAddress(stakingAddress);
            CBsmCoinAddress spendingAddress;
...
#ifdef ENABLE_WALLET
        // Case 1: BsmCoin address and we have full public key:
        CBsmCoinAddress address(ks);
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are bsmcoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) bsmcoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
    ...
    CBsmCoinAddress address(innerID);
...
UniValue createwitnessaddress(const UniValue& params, bool fHelp)
{
    ...
    CBsmCoinAddress address(witscriptid);
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"bsmcoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"bsmcoinaddress\"  (string, required) The bsmcoin address to use for the signature.\n"
...
{
    if (type == 2) {
        address = CBsmCoinAddress(CScriptID(hash)).ToString();
    } else if (type == 1) {
        address = CBsmCoinAddress(CKeyID(hash)).ToString();
    } else {
        return false;
    }
    return true;
...
bool getAddressesFromParams(const UniValue& params, std::vector<std::pair<uint160, int> > &addresses)
{
    if (params[0].isStr()) {
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            CBsmCoinAddress address(it->get_str());
            uint160 hashBytes;
            int type = 0;
            if (!address.GetIndexKey(hashBytes, type)) {
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid address");
...
bool getAddressesFromParamsForHistory(const UniValue& params, std::vector<std::pair<std::pair<uint160, uint160>, AddressHistoryFilter>> &addresses)
{
    if (params[0].isStr()) {
        ...
        {
            CBsmCoinAddress addressVoting, addressStaking, addressSpending;
        ...
        {
            CBsmCoinAddress addressStaking, addressSpending;
        ...
        for (std::vector<UniValue>::iterator it = values.begin(); it != values.end(); ++it) {
            ...
            if (address.IsColdStakingv2Address(Params()))
            {
                CBsmCoinAddress addressSpending, addressVoting, addressStaking;
                uint160 hashBytes, hashBytesSpending, hashBytesVoting, hashBytesStaking;
                int type = 0;
