...
BsmCoinListWidget::BsmCoinListWidget(QWidget *parent, QString title, ValidatorFunc validator) :
    QWidget(parent),
...
void BsmCoinListWidget::onRemove()
{
    listWidget->takeItem(listWidget->row(listWidget->currentItem()));
}

void BsmCoinListWidget::onSelect(QListWidgetItem* item)
{
    removeBtn->setVisible(item != nullptr);
}

void BsmCoinListWidget::onInsert()
{
...
QStringList BsmCoinListWidget::getEntries()
{
    QStringList ret;
