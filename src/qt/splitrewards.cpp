    ...
    comboAddress(new QComboBox)
{
    ...
    {
        comboAddress->insertItem(i++, it.first +" ("+ QString::fromStdString(FormatMoney(it.second)) +" BSM)",it.first);
    }
...
void SplitRewardsDialog::showFor(QString sin)
{
    ...
    for (auto &key: j.keys())
    {
        CBsmCoinAddress address(key.toStdString());
        double amount;
        ...
        if (address.IsValid())
        {
            ...
            if (teamAddresses.count(key))
            {
                if (teamAddresses[key] == "Community Fund")
                    nCFundContribution += PercentageToBsm(amount);
                else
                    descs << QString::fromStdString(FormatMoney(PercentageToBsm(amount))) + " to " + teamAddresses[key];
                jDup[teamAddresses[key]] = amount;
            }
            else {
                descs << QString::fromStdString(FormatMoney(PercentageToBsm(amount))) + " to " + key;
                jDup[key] = amount;
            }
    ...
    strDesc->setText(tr("For each block, %1 BSM will go to the Community Fund, %2 and %3 will be accumulated in your own address").arg(QString::fromStdString(FormatMoney(nCFundContribution)), descs.join(", "), QString::fromStdString(FormatMoney(PercentageToBsm(availableAmount)))));
...
void SplitRewardsDialog::onAdd()
{
    ...
    if (!CBsmCoinAddress(address.toStdString()).IsValid())
    {
        QMessageBox msgBox(this);
        msgBox.setText(tr("Invalid BsmCoin Address"));
        msgBox.addButton(tr("Ok"), QMessageBox::AcceptRole);
        msgBox.setIcon(QMessageBox::Warning);
        msgBox.setWindowTitle("Error");
        msgBox.exec();
