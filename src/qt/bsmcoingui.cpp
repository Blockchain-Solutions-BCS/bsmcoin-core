...
const QString BsmCoinGUI::DEFAULT_WALLET = "~Default";

BsmCoinGUI::BsmCoinGUI(const PlatformStyle *platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    clientModel(0),
    walletFrame(0),
    ...
    fShowingVoting(0)
{
    ...
    for(BsmCoinUnits::Unit u: BsmCoinUnits::availableUnits())
    {
        unitDisplayControl->addItem(QString(BsmCoinUnits::name(u)), u);
    }
...
#ifdef ENABLE_WALLET
    if(enableWallet) {
        connect(walletFrame, &WalletFrame::requestedSyncWarningInfo, this, &BsmCoinGUI::showModalOverlay);
        connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BsmCoinGUI::showModalOverlay);
        connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BsmCoinGUI::showModalOverlay);
    }
...
float BsmCoinGUI::scale()
{
    return GUIUtil::scale();
}

void BsmCoinGUI::createActions()
{
    QActionGroup *tabGroup = new QActionGroup(this);

    overviewAction = new QAction(platformStyle->Icon(":/icons/bsmcoin"), tr("&Overview"), this);
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a BsmCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and bsmcoin: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your BsmCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->IconAlt(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified BsmCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a bsmcoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->IconAlt(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible BsmCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BsmCoinGUI::createHeaderWidgets()
{
...
void BsmCoinGUI::createToolBars()
{
...
void BsmCoinGUI::showOutOfSyncWarning(bool fShow)
{
    showHideNotification(fShow, 1);
}

void BsmCoinGUI::showHideNotification(bool show, int index)
{
    notifications[index]->setVisible(show);
}

void BsmCoinGUI::setActiveMenu(int index)
{
    for (int i = 0; i < 6; ++i)
    {
...
bool BsmCoinGUI::checkSettingsSaved()
{
...
void BsmCoinGUI::setBalance(const CAmount &avail, const CAmount &pendi, const CAmount &immat)
{
    ...
    balanceAvail->setText(BsmCoinUnits::prettyWithUnit(unit, avail, false, BsmCoinUnits::separatorAlways));
    balancePendi->setText(BsmCoinUnits::prettyWithUnit(unit, pendi, false, BsmCoinUnits::separatorAlways));
    balanceImmat->setText(BsmCoinUnits::prettyWithUnit(unit, immat, false, BsmCoinUnits::separatorAlways));
}

void BsmCoinGUI::setStaked(const CAmount &all, const CAmount &today, const CAmount &week)
{
    if (!walletFrame || !clientModel || !clientModel->getOptionsModel())
        return;
    ...
    stakedAvail->setText(BsmCoinUnits::prettyWithUnit(unit, all, false, BsmCoinUnits::separatorAlways));
    stakedPendi->setText(BsmCoinUnits::prettyWithUnit(unit, today, false, BsmCoinUnits::separatorAlways));
    stakedImmat->setText(BsmCoinUnits::prettyWithUnit(unit, week, false, BsmCoinUnits::separatorAlways));
}

void BsmCoinGUI::onDaoEntriesChanged(int count)
{
...
void BsmCoinGUI::setMenuBubble(int index, int drak)
{
...
void BsmCoinGUI::setClientModel(ClientModel *clientModel)
{
...
#ifdef ENABLE_WALLET
bool BsmCoinGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool BsmCoinGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void BsmCoinGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BsmCoinGUI::updateAlerts(const QString &warnings)
{
...
void BsmCoinGUI::setWalletActionsEnabled(bool enabled)
{
...
void BsmCoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void BsmCoinGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void BsmCoinGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void BsmCoinGUI::optionsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BsmCoinGUI::cfundProposalsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BsmCoinGUI::cfundProposalsOpen(bool fMode)
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BsmCoinGUI::cfundPaymentRequestsClicked()
{
    if(!clientModel || !clientModel->getOptionsModel())
        return;
}

void BsmCoinGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BsmCoinGUI::infoClicked()
{
    if(!clientModel)
        return;

    QString link = QString("https://info.bsmcoin.org/");
    QDesktopServices::openUrl(QUrl(link));
}

void BsmCoinGUI::showDebugWindow()
{
...
void BsmCoinGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void BsmCoinGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void BsmCoinGUI::openClicked()
{
...
void BsmCoinGUI::gotoOverviewPage()
{
...
void BsmCoinGUI::gotoHistoryPage()
{
    if (!checkSettingsSaved())
        return;
...
void BsmCoinGUI::gotoCommunityFundPage()
{
    if (!checkSettingsSaved())
        return;
...
void BsmCoinGUI::gotoSettingsPage()
{
    setActiveMenu(5);
...
void BsmCoinGUI::gotoReceiveCoinsPage()
{
    if (!checkSettingsSaved())
        return;
...
void BsmCoinGUI::gotoRequestPaymentPage()
{
    if (!checkSettingsSaved())
        return;
...
void BsmCoinGUI::gotoSendCoinsPage(QString addr)
{
    if (!checkSettingsSaved())
        return;
...
void BsmCoinGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BsmCoinGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
#endif // ENABLE_WALLET

void BsmCoinGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to BsmCoin network", "", count));
}

void BsmCoinGUI::updateHeadersSyncProgressLabel()
{
...
void BsmCoinGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
#ifdef Q_OS_MAC
...
void BsmCoinGUI::showVotingDialog()
{
...
void BsmCoinGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("BsmCoin"); // default title
...
void BsmCoinGUI::changeEvent(QEvent *e)
{
...
void BsmCoinGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BsmCoinGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BsmCoinGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BsmCoinUnits::formatWithUnit(unit, amount, true)) +
                  tr("Type: %1\n").arg(type);
    if (!label.isEmpty())
        msg += tr("Label: %1\n").arg(label);
...
void BsmCoinGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BsmCoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BsmCoinGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BsmCoinGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BsmCoinGUI::setEncryptionStatus(int status)
{
...
void BsmCoinGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BsmCoinGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BsmCoinGUI::detectShutdown()
{
...
void BsmCoinGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void BsmCoinGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void BsmCoinGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BsmCoinGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
void SetBalance(BsmCoinGUI *gui, const CAmount& total, const CAmount& avail, const CAmount &immat)
{
    // Call our instance method
    gui->setBalance(total, avail, immat);
}

void SetStaked(BsmCoinGUI *gui, const CAmount& all, const CAmount& today, const CAmount &week)
{
    // Call our instance method
    gui->setStaked(all, today, week);
}

void BsmCoinGUI::subscribeToCoreSignals()
{
...
void BsmCoinGUI::unsubscribeFromCoreSignals()
{
...
void BsmCoinGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
...
void BsmCoinGUI::updateDisplayUnit(int unit)
{
    // Update the list value
    unitDisplayControl->setCurrentText(BsmCoinUnits::name(unit));
}

/** Update the display currency **/
void BsmCoinGUI::comboBoxChanged(int index)
{
...
void BsmCoinGUI::updatePrice()
{
    ...
    std::thread pThread{[this]{
        try {
            ...
            std::string response;
            std::string url(
                    "https://api.coingecko.com/api/v3/simple/price?ids=bsm-coin&vs_currencies="
                    "BTC,"
                    "EUR,"
                    "USD,"
                    "ARS,"
            ...
            boost::property_tree::ptree json = _json.get_child("bsm-coin");
...
size_t BsmCoinGUI::priceUdateWriteCallback(void *contents, size_t size, size_t nmemb, void *userp)
{
    ((std::string*) userp)->append((char*) contents, size * nmemb);
    return size * nmemb;
}

void BsmCoinGUI::updateStakingStatus()
{
    updateWeight();
