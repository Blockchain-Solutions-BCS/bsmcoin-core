...
- Fee to create a Proposal: 50 BSM
...
You can read more about [NPIP0003 on the NPIP GitHub](https://github.com/bsmcoin/npips/blob/master/npip-0003.mediawiki).

## Community Fund Contribution Increase

The Version Bit 16 will signal for the soft fork to increase the Community Fund contribution from 0.25 BSM to 0.50 BSM per block. 

You can read more about [NPIP0005 on the NPIP GitHub](https://github.com/bsmcoin/npips/blob/master/npip-0005.mediawiki).
...
```
createproposal bsmcoinaddress amount duration "desc" fee

Arguments:
1. "bsmcoinaddress" (string, required) The bsmcoin address where coins would be sent if the proposal is approved.
2. "amount" The amount in BSM to request. eg 100
...
2. "amount" (numeric or string, required) The amount in BSM to withdraw. eg 10
...
Donate BSM from your wallet to the commnuity fund

Arguments:
1. "amount" (string, required) The amount of BSM to donate
```
...
### Other modifications in the BsmCoin client:
