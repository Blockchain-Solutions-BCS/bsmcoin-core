...
###   This script attempts to download the signature file SHA256SUMS.asc from bsmcoin.org
...
WORKINGDIR="/tmp/bsmcoin"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://bsmcoin.org/bin/"
VERSIONPREFIX="bsmcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bsmcoin-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the BsmCoin Core binary release signing key installed?"
   fi
