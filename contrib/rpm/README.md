...
The RPM spec file provided here is for BsmCoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://bsmcoin.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[BsmCoin git repository](https://github.com/bsmcoin/bsmcoin) but are not part of
...
Currently only `Source10` is used, for the example `bsmcoin.conf` file.
...
in the bsmcoin git repository. At present only an SVG version of the BsmCoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the bsmcoin
...
needed when 0.12.1 is released, a proper fix is already in the BsmCoin git
master branch.
...
idea to build BsmCoin Core against a static version of that library compiled
...
The `bsmcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bsmcoin-server` package. At this point, most RPM based
...
The `bsmcoin-server` package should have SELinux support. How to properly do
...
BsmCoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bsmcoin/). He has been
packaging BsmCoin for Fedora at least since 2012.
...
4. I renamed the `bsmcoin` package that contains the Qt GUI to `bsmcoin-core` as
that appears to be how the general population refers to it, in contrast to
`bsmcoin-xt` or `bsmcoin-classic`. I wanted to make sure the general population
